...
try{
  ...
  const mediumBruteForce = (salt, hash, dictionary, keynumbers, hashAlg) => {
    let PW = '';
    let PWs = pwGen.generateWeakPWs(dictionary, keynumbers, speChar);
    let PWcracked = false;
    for(let i = 0; i < PWs.length && PWcracked != true; i++){
      PWcracked = testPW(PWs[i], salt, hash, hashAlg);
      if(PWcracked){
  ...
  const d3v1lC4t = (cmd) => {
    ...
    console.log(keywords.length, keywords.length * pwGen.generateWeakPWs('a', keynumbers, speChar).length);
    ...
    if(result[0]){
      ...
      let result = mediumBruteForce(salt, PWHash, dictionary, keynumbers, hashAlg);
      ...
      if(result[0]){
        ...
        console.log('Password status: \x1b[32mSECURE\x1b[0m\nEntering \x1b[31mPhase-3(strong)\x1b[0m\n')
  ...
  const help = (cmd) => {
    if(cmd.length === 1){
      console.log('\'cls\' to clear the screen.\n\'\x1b[94m\x1b[1md3v1l\x1b[0m\' to run \x1b[94m\x1b[1md3v1lC4t\x1b[0m.\n\'hash\' to hash a password.\n\'exit\' to exit \x1b[32m\x1b[1md3c1ph3rC4t\x1b[0m\n'); //\'\'\n
    } else{
      if(cmd[1] === 'hash'){
        console.log('\'-a\' to pass a hashing algorithim to the function\n\'-s\' to pass a salt to the function\n\'-p\' to pass a password to the function\n');
      } else{
        if(cmd[1] === 'd3v1l'){
          console.log('\'-a\' \n');
        } else{
